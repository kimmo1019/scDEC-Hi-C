...
class scATAC_Sampler(object):
    def __init__(self,name='GMvsHL',dim=20):
        ...
        print(uniq_labels)
        print([labels.count(item) for item in uniq_labels])
        sys.exit()
        ...
        if False:
            ...
            dic_cluster = {i:np.load('donor_effect/data_k=%d.npz'%i)['arr_1'] for i in range(2,7)}

            ratios,ratios_per_cluster = compute_ratio(dic_cluster, X, k_max)
            #ratios,ratios_per_cluster = compute_ratio(SpectralClustering(random_state=10,assign_labels='kmeans'), X, k_max)
            #ratios,ratios_per_cluster = compute_ratio(KMeans(), X, k_max)
            print(ratios_per_cluster)
            ratios_by_cluster = [np.mean(each) for each in ratios_per_cluster]
            #print([np.mean(each) for each in ratios_per_cluster])
            #ratios = compute_ratio(KMeans(),X,k_max)
            #ratios = compute_ratio(SpectralClustering(random_state=100,assign_labels='kmeans'), X, k_max)
            print(ratios)
            plt.plot(x_axis, ratios, '-o',color='b')
            plt.plot(x_axis, ratios_by_cluster, '-^',color='r')
...
if __name__=='__main__':
    ...
    sys.exit()
    X = pd.read_csv('datasets/scATAC/%s/sc_mat.txt'%name,sep='\t',header=0,index_col=[0])
    X = X.astype(int)
    X.to_csv('datasets/scATAC/%s/sc_mat_v2.txt'%name,sep='\t')
    sys.exit()
